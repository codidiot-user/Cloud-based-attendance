...
class SLM_Assistant:
    def __init__(self, db_connection):
        ...
        # --- FINALIZED PROMPT WITH STRONGER EXAMPLE ---
        self.system_prompt = """
    ...
    def _get_ai_decision(self, user_question):
        ...
        try:
            ...
            # --- ADDED DEBUG LINE ---
            print(f"DEBUG: Raw AI Response: '{decision_str}'")
            # ------------------------

            # Parse the simple "tool,date" string
            parts = decision_str.split(',')
            tool = parts[0].strip()
            # Handle potential index error if AI response is malformed
    ...
    def process_question(self, user_question: str) -> str:
        ...
        try:
            ...
            if tool == "get_present":
                ...
                # Safely handle potential None result before accessing index
        ...
        except Exception as err: # Catch potential database errors more broadly
    ...
    def _get_all_students(_self):
        ...
        from psycopg2.extras import DictCursor # Import locally if needed
        cursor = _self.db_connection.cursor(cursor_factory=DictCursor)
...
# --- Need to import DictCursor at the top ---
# Make sure this import is present at the very top of the file
